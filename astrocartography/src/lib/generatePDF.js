...
// ORDINALS now come from i18n: t('ord1', lang), t('ord2', lang), etc.
...
function cityBlock(d, y, cities, color, readingFn, pw, ph, max, lang) {
  const list = cities.slice(0, max);
  if (!list.length) {
    d.setFont('helvetica', 'italic'); d.setFontSize(8); sc(d, C.textDim);
    y = cp(d, y, 6, pw, ph);
    d.text(t('pdfNoCities', lang), 20, y);
    return y + 8;
  }
...
// ══════════════════════════════════════════
export async function generateNatalPDF({ displayName, chartData, thriveC, avoidC, neutralC, cityReadingFn, natalReadings, lang = 'en' }) {
  ...
  d.text(t('pdfTitle', lang), pw / 2, y, { align: 'center' });
  y += 10;
  d.setFontSize(11); sc(d, C.accent);
  d.text(t('pdfSubtitleReport', lang), pw / 2, y, { align: 'center' });
  y += 14;
  ...
  d.text(`${t('pdfGenerated', lang)} ${new Date().toLocaleDateString(lang === 'de' ? 'de-DE' : lang === 'fr' ? 'fr-FR' : lang === 'it' ? 'it-IT' : lang === 'es' ? 'es-ES' : lang === 'tr' ? 'tr-TR' : lang === 'ru' ? 'ru-RU' : 'en-US', { year: 'numeric', month: 'long', day: 'numeric' })}`, pw / 2, y, { align: 'center' });
  y += 4;
  d.text('natalnavigator.com', pw / 2, y, { align: 'center' });

  // ── NATAL CHART TABLE ──
  d.addPage(); fr(d, 0, 0, pw, ph, C.bg); y = 20;
  y = sh(d, y, t('pdfYourChart', lang), pw, ph);

  fr(d, 16, y, pw - 32, 7, C.panel);
  d.setFont('helvetica', 'bold'); d.setFontSize(7); sc(d, C.textDim);
  d.text(t('pdfPlanetCol', lang), 20, y + 5); d.text(t('pdfSignCol', lang), 60, y + 5); d.text(t('pdfDegreeCol', lang), 100, y + 5);
  d.text(t('pdfHouseCol', lang), 130, y + 5); d.text(t('pdfElementCol', lang), 158, y + 5);
  y += 9;
  ...
  if (chartData?.natal) {
    ...
    y += 5;
    for (const [label, data] of [[t('ascendant', lang), chartData.natal.asc], [t('midheaven', lang), chartData.natal.mc]]) {
      if (!data) continue;
  ...
  // ── PLANETARY READINGS ──
  y += 4;
  y = sh(d, y, t('pdfReadings', lang), pw, ph);
  ...
  if (chartData?.planets && natalReadings) {
    for (const p of chartData.planets) {
      ...
      d.text(`${p.id} ${t('inThe', lang).toLowerCase()} ${p.sign}`, 20, y + 5);
      if (p.house) {
        d.setFontSize(8); sc(d, C.textDim);
        d.text(`${t(`ord${p.house}`, lang)} ${t('houseWord', lang)}`, pw - 20, y + 5, { align: 'right' });
      }
      y += 12;
      ...
      if (sr) { y = sub(d, y, `${p.id.toUpperCase()} ${t('inThe', lang)} ${p.sign.toUpperCase()}`, PCOL[p.id], pw, ph); y = bt(d, y, sr.text, pw, ph, 18, true); y += 1; }
      if (hr) { y = sub(d, y, `${p.id.toUpperCase()} ${t('inThe', lang)} ${t(`ord${p.house}`, lang).toUpperCase()} ${t('houseWord', lang)}`, PCOL[p.id], pw, ph); y = bt(d, y, hr.text, pw, ph, 18, true); }
      y += 4;
    }
  }
  ...
  if (chartData?.natal?.asc && natalReadings?.ASC_IN_SIGN) {
    ...
    if (r) {
      ...
      d.text(`${t('pdfAscendantIn', lang)} ${chartData.natal.asc.sign}`, 20, y + 5);
  ...
  if (chartData?.natal?.mc && natalReadings?.MC_IN_SIGN) {
    ...
    if (r) {
      ...
      fr(d, 16, y - 1, pw - 32, 9, C.panel);
      d.setFont('helvetica', 'bold'); d.setFontSize(10); sc(d, '#E8A838');
      d.text(`${t('pdfMidheavenIn', lang)} ${chartData.natal.mc.sign}`, 20, y + 5);
      y += 12; y = bt(d, y, r.text, pw, ph, 18, true); y += 4;
    }
  }

  // ── CITIES ──
  y = sh(d, y, t('pdfThriveZones', lang), pw, ph);
  y = cityBlock(d, y, thriveC, C.thrive, cityReadingFn, pw, ph, 20, lang);
  y = sh(d, y, t('pdfCautionZones', lang), pw, ph);
  y = cityBlock(d, y, avoidC, C.avoid, cityReadingFn, pw, ph, 15, lang);
  y = sh(d, y, t('pdfNeutralZones', lang), pw, ph);
  y = cityBlock(d, y, neutralC, C.neutral, cityReadingFn, pw, ph, 10, lang);
  ...
  y += 8;
  d.setFont('helvetica', 'normal'); d.setFontSize(9); sc(d, C.textDim);
  d.text(t('pdfFindStars', lang), pw / 2, y, { align: 'center' });
  y += 12;
